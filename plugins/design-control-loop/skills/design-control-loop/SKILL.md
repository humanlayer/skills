...
description: help the user define a measurable improvement goal, a checker, and a rule for selecting the next reviewable change, then build the resulting coding-agent loop as locally runnable components plus a scheduled workflow; use build-iterated-agentic-loop when those decisions are already made
---
...
This is the design-first option. Use `build-iterated-agentic-loop` instead when the recurring task, scope, and validation are already defined and the user mainly needs the skill and workflow installed. The two skills create some of the same automation artifacts; this one adds the discovery and design needed when the loop itself is not yet clear.
...
A loop improves the codebase through repeated, reviewable changes instead of one large rewrite:

- **Goal** — the desired state for some property of the codebase.
- **Checker** — measures the current state and reports the remaining gap.
- **Selector** — chooses the next small, low-risk change from that report.
- **Agent** — applies the change and opens a PR.
- **External changes** — teammates' commits, dependency updates, generated code, and anything else that can move the codebase while the loop runs.
- The result informs the next run, while a human continues to steer the loop.

Read `references/control-loop-taxonomy.md` and walk the user through these concepts before designing anything. It includes the equivalent control-theory terms for readers who find them useful, but use the plain-language terms with the user by default. For one fully worked example, see `references/example-control-loop.md` — treat it as an illustration, not a blueprint.

## How to run this skill

- **Read the repo before you ask** (Phase A). Come to the interview with proposals, not a blank form.
- **Tailor every component.** The right checker, selector, and agent depend entirely on the user's problem and stack. The lists in the references are examples to spark discussion, never a checklist to push.
...
- The **checker** and **selector** as version-controlled commands/scripts the user can run locally.
- `.claude/skills/<skill-name>/SKILL.md` — the coding-agent skill capturing the agent's judgement (path may be `.agents/skills/...` per repo convention).
...
- Optionally, a **regression gate** that keeps the problem from getting worse while the loop improves it.
...
- The static-analysis, linting, codegen, and test tooling already in the repo — these are the most likely raw material for a checker.
...
This is an interview. Work through each component below. Start by asking the user questions about the goal. Propose options grounded in Phase A and surface trade-offs rather than mandating any choice. Record the decisions as you go.

1. **Goal.** What property are we improving, and to what target? Examples: an invariant ("no procedures use the old pattern"), a threshold ("test coverage ≥ X in these packages"), or a direction ("reduce occurrences each run"). Also pin the **scope**: which directories/packages the loop may change, and which it may only read.

2. **Checker.** How will the loop measure the gap to the goal? Inspect the codebase and the user's existing tooling and propose the options that fit *their* stack — a static-analysis or lint tool, a structural/AST search, a test suite, a type checker, a telemetry or error query, a custom script, or even an agent-based check. Discuss the trade-offs that matter to them (stability, cost, repeatability, and whether the measurement can be silently disabled) instead of mandating any property. Aim for a report the selector can act on repeatably.

3. **Selector.** How will the loop choose the next increment from the checker's report, sized to stay low-risk and reviewable? Design this *with* the user: how to prioritize targets, how big one increment is, and what "one reviewable unit of work" means here. A selector can be anything from fully deterministic (a script that sorts findings and picks the next target) to fully agentic (an agent that decides from natural-language criteria), and it may be combined with the checker or coding agent. The selector is the part you will **tune over time** from loop output — start simple and expect to revise it.

4. **Agent.** A coding agent plus a repo-local skill applies the change.
   ...
   - **Golden patterns first.** Before automating, establish what a good change looks like: ask the user whether existing patterns in the codebase should be followed, and inspect the code to find them. Capture these in the agent skill (Phase C).
   - **Validation.** Decide which commands must pass before the agent commits (propose these from Phase A and confirm).

5. **External changes + regression gate (offer).** Name what changes the codebase outside the loop (teammates shipping concurrently, dependency bumps, generated code). Then **offer** a regression gate: a check that keeps the measured problem from getting worse while the scheduled loop chips away at it — for example a PR check that compares the checker's output against a baseline and surfaces (or eventually blocks) newly introduced deviations. This is optional; some loops do not need one.

Completion criterion: a short written design naming the goal, checker, selector, coding agent (including its skill and validation), external changes, and optional regression gate — with each active component something the user can run locally.

### Phase C — Build the coding-agent skill

**Read the following references:** `references/skill-template.md`, `references/example-skill.md`, `references/response-template.md`.

Write a repo-local skill that captures the coding agent's judgement for this task. It can use repo-specific paths, package names, and conventions since it lives in the repository.
...
Before any CI exists, land the checker and selector as version-controlled commands or scripts (follow the repo's convention for where such scripts live), and verify the whole loop works by hand:

- Run the **checker** standalone and confirm it produces a stable, usable report.
- Run the **selector** on real checker output and confirm it selects a sensible next increment.
- Run the **agent** locally via its headless CLI command on a selector-chosen target, and confirm it makes the change and passes validation.
...
Completion criterion: the user can run the checker, selector, and agent locally and independently.
...
- Run the loop as **discrete steps: checker → selector → agent**, then commit and open a PR using the agent's final message as the body. (When components are combined — e.g. the checker already prioritizes, or one agent both selects and changes — collapse them into a single step; do not invent separation the design does not have.)
...
- Interpolate the memory file (Phase F) into the agent's context.
...
- **Memory/feedback file.** A version-controlled markdown file (e.g. `.github/agent-memory/<task-slug>.md`) loaded deterministically into the agent's context **after the selector** on every run. Use `references/memory-template.md`. Good entries: permanent scope exclusions, known false-positive areas, and reviewer feedback that should change future selections — not one-off instructions or single-run logs.
...
Frame this for the user as **how you tune the selector and skill over time** — the loop gets better because a human keeps correcting it.
...
**Ready to iterate faster** (once the loop is tuned and producing consistent, high-quality output): increase the schedule frequency; widen the selector's batch (e.g. select N targets per run); run the check→select→change cycle N times per workflow run; or run the workflow multiple times and assign one PR to each teammate.
...
- `references/control-loop-taxonomy.md` — the plain-language loop components, optional control-theory mappings, and design questions to ask; read this first and use it to guide the user.
...
- `references/workflow-template.yml` — recurring loop workflow skeleton with discrete checker/selector/agent steps.
- `references/prompt-template.md` — embedded prompt structure for the coding-agent step.
- `references/memory-template.md` — memory/feedback file skeleton.
- `references/skill-template.md` — skeleton for the generated coding-agent skill.
